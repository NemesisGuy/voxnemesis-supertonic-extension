...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Surface asset download/caching status from the offscreen worker.
    function handleAssetStatus(msg) {
        if (msg.phase === 'start') {
            showOverlay(msg.message || 'Fetching resources from Hugging Face...');
        } else if (msg.phase === 'download') {
            const detail = msg.status === 'cached' ? 'cached' : 'downloading';
            showOverlay(`${msg.file || 'Model'}: ${detail}...`);
        } else if (msg.phase === 'ready') {
            hideOverlay();
            updateStatus(msg.message || 'Models cached. Ready to read.');
        } else if (msg.phase === 'error') {
            showOverlay(msg.message || 'Model download failed. Check your connection.', true);
            updateStatus(msg.message || 'Model download failed. Check your connection.');
        }
    }
    ...
    function showOverlay(text, isError = false) {
        ...
        overlay.classList.remove('hidden');
        if (isError) {
            overlayText.style.color = '#fca5a5';
        } else {
            overlayText.style.color = '#f8fafc';
        }
    }
